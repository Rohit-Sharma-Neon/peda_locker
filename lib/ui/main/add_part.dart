...
class _AddPartState extends State<AddPart> {
  ...
  setData() {
    if (widget.isEditing) {
      ...
      selectedPartsId = widget.bikeData?.partsId;
      selectedPartName = widget.bikeData?.parts;
    }
  }
    ...
    return Scaffold(
      ...
      body: Column(
        children: [
          OnBoardingHeader(
              heading:
                  widget.isEditing ? "selectParts".tr() : "selectParts".tr(),
                                ...
                                onChanged: (newValue) {
                                  ...
                                  if (newValue != null) {
                                    setState(() {
                                      ...
                                      for (var a in partsData.data!) {
                                        if (a.name == selectedPartName) {
                                          selectedPartsId = a.id.toString();
                                        }
                                      }
                                      print("sfasdfsadfadsf" +
                                          selectedPartsId.toString());
                                    });
                                  }
                                },
                                items: partsName.map((location) {
    ...
    await apis.partsApi().then((value) {
      ...
      if (value?.status ?? false) {
        ...
        partsData.data?.forEach((parts) {
          partsName.add(parts.name);
        });
        // if (partsData.data!.length > 10) {
        //   for (var i = 0; i < 10; i++) {
        //     partsName.add(partsData.data![i].name);
        //   }
        // } else {
        //   partsData.data?.forEach((parts) {
        //     partsName.add(parts.name);
  ...
  addPartsApi() async {
    await apis
        .addPartsApi(
      partId: selectedPartsId.toString(),
