...
import 'package:cycle_lock/ui/main/order_details.dart';
...
class AllOrdersScreen extends StatefulWidget {
  ...
  AllOrdersScreen({Key? key, this.isUnlock = false, this.type = 0})
      : super(key: key);
    ...
    apis.orderListApi("all").then((value) {
      ...
      if (value?.status ?? false) {
        setState(() {
          ...
          cancellationCharges = value?.cancellationCharges ?? "0";
        });
      } else {
        setState(() {
          allOrdersList = [];
      ...
      child: Scaffold(
        ...
        body: Container(
          ...
          child: isLoading
              ...
              : allOrdersList != null && allOrdersList?.length != 0
                  ? ListView.builder(
                      shrinkWrap: true,
                  ...
                  children: [
                    ...
                    Row(children: [
                      ...
                      Text(
                         ...
                         //"${DateFormat(dateFormatSlash).format(DateTime.parse(allOrdersList?[index].checkInDate.toString() ?? ""))}\n${DateFormat(dateFormatSlash).format(DateTime.parse(allOrdersList?[index].extend_date_time.toString()??allOrdersList?[index].checkOutDate.toString() ?? "")) }",
                          ...
                          "${allOrdersList?[index].checkInDate.toString() ?? ""}\n${allOrdersList?[index].extend_date_time != null ? dateTime(allOrdersList?[index].extend_date_time) : allOrdersList?[index].checkOutDate.toString() ?? ""}",
                          style: const TextStyle(
                              color: Colors.black,
                              fontWeight: FontWeight.w500,
                              fontSize: ts20)),
                      const Spacer(),
                      // allOrdersList?[index].is_assgin_lock == 0
                      //     ? GestureDetector(
                      //         onTap: () {
                      //           var bikeIds = "";
                      //           for (var a in allOrdersList![index].bike!) {
                      //             if (bikeIds == "") {
                      ...
                      //                       bookingId: allOrdersList![index].id.toString(),
                      //                       check_in_date: allOrdersList![index].checkInDate,
                      //                       check_out_date: allOrdersList![index].checkOutDate,
                      //                       bikeIds: bikeIds,
                      //                       soptId:
                      //                       allOrdersList?[index].parkingSpotId?.toString() ?? "")));
                      //         },
                      //         child: const Icon(
                      //           Icons.edit_outlined,
                      //           size: 30,
                      //         ),
                      //       )
                      //     : SizedBox(), allOrdersList?[index].is_assgin_lock == 0
                      //     ? GestureDetector(
                      //         onTap: () {
                    ...
                    Row(
                        ...
                        children: [
                          Row(children: [
                            ...
                            allOrdersList?[index].bike != null &&
                                ...
                                ? Container(
                                    ...
                                    child: Text(
                                      "+" +
                                          (allOrdersList![index].bike!.length -
                                                  1)
                                              .toString(),
                                      style: const TextStyle(
                                          color: Colors.white,
                          ...
                          Text(
                              "AED ${double.parse((allOrdersList?[index].totalAmount ?? "").replaceAll(",", "")).toStringAsFixed(2)}",
                              style: const TextStyle(
                                  color: Colors.black,
                                  fontWeight: FontWeight.w500,
                                  fontSize: ts22)),
                    ...
                    allOrdersList?[index].bike != null &&
                            allOrdersList![index].bike!.length > 1
                        ? const SizedBox()
                        : allOrdersList?[index].is_assgin_lock == 1 &&
                                allOrdersList?[index].orderStatus.toString() !=
                                    "cancel"
                            ? Column(
                                mainAxisAlignment: MainAxisAlignment.start,
                                crossAxisAlignment: CrossAxisAlignment.start,
                            ...
                            children: [
                              ...
                              Row(
                                children: [
                                  ...
                                  if(allOrdersList?[index].orderStatus.toString() == "order")
                                  const Spacer(),
                                  if(allOrdersList?[index].orderStatus.toString() == "order")
                                  InkWell(
                                    onTap: (){
                        ...
                        : allOrdersList?[index].orderStatus.toString() ==
                            ...
                            ? Row(
                                mainAxisAlignment:
                                    MainAxisAlignment.spaceBetween,
                                children: [
                                 allOrdersList![index].rating.toString().isEmpty?Expanded(
                                      child: GestureDetector(
                                        onTap: () {
                                          showRateDialog(
                                              context,
        ...
        Navigator.push(
            ...
            MaterialPageRoute(
                builder: (context) => BookingShareScreen(
                      orderId: allOrdersList?[id].id?.toString() ?? "",
                      checkIn: allOrdersList?[id].checkIn?.toString() ?? "",
